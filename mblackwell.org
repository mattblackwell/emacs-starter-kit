...
#+begin_src emacs-lisp
   ...
   (add-hook 'poly-markdown+r-mode-hook
             '(lambda()
                (local-set-key (kbd "A-K") 'rmarkdown-render-file)
                (local-set-key (kbd "C-c C-c v") 'rmarkdown-view-pdf)))
   (eval-after-load "poly-markdown+r-minor-mode"
     '(define-key markdown-mode-map (kbd "A-K") 'rmarkdown-render-file))
#+end_src
...
".rel" ".fls" ".run.xml"))
  ...
  (setq flychceck-lintr-linters "with_defaults(multiple_dots_linter = NULL)")
